...
serve_read(envid_t envid, union Fsipc *ipc)
{
	...
	int res = openfile_lookup(envid, req -> req_fileid, &file);
	if (res) return res;
	res = file_read(file -> o_file, ret -> ret_buf, req -> req_n, file -> o_fd -> fd_offset);
	if (res < 0) return res;
	file -> o_fd -> fd_offset += res;
	return res;
}
...
serve_write(envid_t envid, struct Fsreq_write *req)
{
	...
	int res = openfile_lookup(envid, req -> req_fileid, &file);
	if (res) return res;
	res = file_write(file -> o_file, req -> req_buf, req -> req_n, file -> o_fd -> fd_offset);
	if (res < 0) return res;
	file -> o_fd -> fd_offset += res;
	return res;
}
